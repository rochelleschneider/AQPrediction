...
data <- merge(train, covar, all.by="id")
data <- merge(data, locat, all.by="id")
class(data$time)
data$dow <- weekdays.POSIXt(data$time)
data$hour <-  ??? (data$time) # do you remember how to extract the hour??
head(d)
nrow(data)
...
#SPACE CV
space.cv <- CreateSpacetimeFolds(data, spacevar = "id")
...
#\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
#TIME CV
time.cv <- CreateSpacetimeFolds(data, spacevar = "time")
...
#\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
#SPACE-TIME CV
spacetime.cv <- CreateSpacetimeFolds(data, spacevar = "id",timevar ="time")
...
lm.spacetime <- lm(PM10 ~ pred, data=spacetime)
